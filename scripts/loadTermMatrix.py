...
def vectorize_so_posts(post_samples, log_batch_size=100, verbose=True):
	...
	for i,question in enumerate(post_samples):  #Temporarily filter to only questions
		...
		doc_text = title_keyword_weight*title +' '+ flatten_markdown(question.body) +' '+ flatten_markdown(answer)
...
if __name__=='main':
	...
	tags_to_include = ['javascript','iphone','mysql','arrays','tomcat']
	tag_objs = UniqueTags.objects.filter(tag_text__in=tags_to_include)
	post_ids = TaggedPosts.objects.filter(tag__in=tag_objs).select_related('post').values_list('post',flat=True)
	qs = Posts.objects.filter(post_type_id__in=[1], id__in=post_ids)  #Comment to take all posts
	...
	#Is this part really necessary? I don't think so
	'''
	...
	for n_clusters in cluster_sizes:
		...
		km = AgglomerativeClustering(n_clusters=n_clusters, affinity='euclidian',linkage='complete')
		...
		#Get cluster centroids and use them for dendrogram
	...
	'''
	X_sub = km.cluster_centers_
	dist_matrix = pdist(X_sub, 'cosine') #compute the cosine distance
	linkage_matrix = linkage(dist_matrix, method='complete') #Furthest point linkage might minimize unrelated questions on the edges
	#Could also use centroid linkage



	f = plt.figure(101)
	f.title("ascending")
	dendrogram(linkage_matrix, color_threshold=1, p = 30, truncate_mode='lastp', distance_sort='descending')
	f.gca().set_ylim([0.9,1.1])
	f.show()
